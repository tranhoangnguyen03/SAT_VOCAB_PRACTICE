...
class MainEngine:
  ...
  def get_definition(self, word):
    ...
    soup = BeautifulSoup(response.content, "html.parser" ).find("div", class_= "VpH2eb vmod XpoqFe")
    if not soup:
  ...
  def score(self, words, sentence):
    ...
    if '-' in words:
      return np.sum([i in ' '.join([j[:j.find('-')].strip().upper() for j in words]) for i in re.split(r'\W+', sentence.upper())])
    else:             
      return np.sum([i in ' '.join(words) for i in re.split(r'\W+', sentence.upper())])
